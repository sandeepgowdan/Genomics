...
write.csv(pheno_data_2, "phenotypedata.csv",row.names = FALSE)
write.csv(geno7, "genotypedata.csv",row.names = FALSE)

gen
geno7 <- rownames(geno7) <- NULL


# Get the rows names of both files
rows_pheno <- rownames(pheno_data_2)
rows_geno <- rownames(geno7)

# Check if the column names are the same
if (identical(rows_pheno, rows_geno)) {
...
pheno_data_3 <- read.csv("phenotypedata.csv")
geno8 <- read.csv("genotypedata.csv")



# Get the rows names of both files
rows_pheno <- rownames(pheno_data_3)
rows_geno <- rownames(geno8)

# Find the differences in row names
diff_in_pheno <- setdiff(rows_pheno,rows_geno)
diff_in_geno <- setdiff(rows_geno, rows_pheno)

# Print the differences
if (length(diff_in_pheno) == 0 && length(diff_in_geno) == 0) {
  print("Row names are the same.")
} else {
  if (length(diff_in_pheno) > 0) {
    print("Row names in file1 but not in file2:")
    print(diff_in_pheno)
  }
  if (length(diff_in_geno) > 0) {
    print("Row names in file2 but not in file1:")
    print(diff_in_geno)
  }
}
